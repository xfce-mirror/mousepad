...
struct _TestPlugin
{
  ...
  /* target data to know more about the application state */
...
{
  plugin->action_name = NULL;
  plugin->dialog_type = G_TYPE_INVALID;
...
{
  test_plugin_disable (MOUSEPAD_PLUGIN (object));
  ...
  for (window = windows; window != NULL; window = window->next)
    {
      ...
      if (G_TYPE_CHECK_INSTANCE_TYPE (dialog, plugin->dialog_type)
                          ...
                          TestPlugin   *plugin)
{
  ...
  if (g_strcmp0 (action_name, plugin->action_name) == 0)
    {
      action = g_action_map_lookup_action (G_ACTION_MAP (action_group), plugin->action_name);
      g_signal_connect_swapped (action, "activate",
                                G_CALLBACK (test_plugin_dialog_shown), plugin);
    }
}
...
static void
test_plugin_open (TestPlugin *plugin)
{
  /* `mousepad --encoding -- file` */
  plugin->dialog_type = MOUSEPAD_TYPE_ENCODING_DIALOG;
  g_timeout_add_seconds (2, test_plugin_dialog_shown, plugin);
}
...
{
  TestPlugin   *plugin = TEST_PLUGIN (mplugin);
  GApplication *application;
  ...
  g_signal_connect_object (application, "open", G_CALLBACK (test_plugin_window_shown), mplugin,
                           G_CONNECT_AFTER | G_CONNECT_SWAPPED);
  g_signal_connect_swapped (application, "open", G_CALLBACK (test_plugin_open), mplugin);
  plugin->action_name = "preferences";
  plugin->dialog_type = MOUSEPAD_TYPE_PREFS_DIALOG;
