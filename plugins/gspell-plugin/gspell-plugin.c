...
G_DEFINE_DYNAMIC_TYPE (GspellPlugin, gspell_plugin, G_TYPE_OBJECT);
...
static gint
gspell_plugin_compare_views (gconstpointer item_data,
                            ...
                            GtkWidget    *mousepad_view)
{
  ...
  item = g_list_find_custom (plugin->views, mousepad_view, gspell_plugin_compare_views);
                              ...
                              GtkWidget    *widget)
{
  ...
  if (plugin->views != NULL)
    item = g_list_find_custom (plugin->views, document->textview, gspell_plugin_compare_views);
...
static void
gspell_plugin_enable (GspellPlugin *plugin)
{
...
static void
gspell_plugin_disable (GspellPlugin *plugin)
{
  GspellPluginView    *view;
  MousepadApplication *application;
  GList               *list, *item;
...
static void
gspell_plugin_state_changed (GspellPlugin *plugin)
{
  gchar    **plugins;
  gboolean   contained;
  ...
  contained = g_strv_contains ((const gchar *const *) plugins, "mousepad-plugin-gspell");

  /* essentially re-init the plugin, but without recreating costly spell checkers */
  if (! plugin->enabled && contained)
    {
      plugin->enabled = TRUE;
      gspell_plugin_enable (plugin);
    }
  /* disconnect all signal handlers and disable spell checkers */
  else if (plugin->enabled && ! contained)
    {
      plugin->enabled = FALSE;
      gspell_plugin_disable (plugin);
    }
...
gspell_plugin_init (GspellPlugin *plugin)
{
  ...
  /* bind to gsettings to keep the plugin state in sync */
  MOUSEPAD_SETTING_CONNECT_OBJECT (ENABLED_PLUGINS, G_CALLBACK (gspell_plugin_state_changed),
                                   plugin, G_CONNECT_SWAPPED);
                              ...
                              GtkWidget    *menu)
{
  ...
  view = g_list_find_custom (plugin->views, mousepad_view, gspell_plugin_compare_views)->data;
