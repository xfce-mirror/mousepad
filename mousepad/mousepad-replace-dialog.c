  ...
  if (G_LIKELY (g_signal_lookup ("reverse-activate", GTK_TYPE_ENTRY) == 0))
    {
      ...
      gtk_binding_entry_add_signal (binding_set, GDK_Return, GDK_SHIFT_MASK, "reverse-activate", 0);
      gtk_binding_entry_add_signal (binding_set, GDK_KP_Enter, GDK_SHIFT_MASK, "reverse-activate", 0);
    }
...
mousepad_replace_dialog_init (MousepadReplaceDialog *dialog)
{
  ...
  g_signal_connect (G_OBJECT (dialog->search_entry), "activate", G_CALLBACK (mousepad_replace_dialog_entry_activate), dialog);
  g_signal_connect (G_OBJECT (dialog->search_entry), "reverse-activate", G_CALLBACK (mousepad_replace_dialog_entry_reverse_activate), dialog);
