...
void
mousepad_util_set_real_line_offset (const GtkTextIter *end_iter,
                                    gint               tab_size,
                                    gint               column,
                                    gboolean           from_end)
{
  ...
  /* forward the needle until we hit the end_iter */
  while (!gtk_text_iter_equal (&needle, end_iter) && column_offset < column)
    {
  ...
  gtk_text_iter_set_line_offset (end_iter, char_offset);
}
