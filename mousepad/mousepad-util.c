...
mousepad_util_set_titlebar (GtkWindow *window)
{
  ...
  /* do not change "show-close-button" if there is extra widgets, this can be handled
   * by one of them or not */
  else if ((list = gtk_container_get_children (GTK_CONTAINER (bar))) != NULL)
    {
  ...
  /* try to retrieve the window title, or use the application name as default */
  title = gtk_window_get_title (window);
  if (title != NULL && *title != '\0')
    gtk_header_bar_set_title (GTK_HEADER_BAR (bar), title);
  else
    gtk_header_bar_set_title (GTK_HEADER_BAR (bar), g_get_application_name ());
