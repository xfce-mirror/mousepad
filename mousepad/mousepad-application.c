                                      ...
                                      gpointer       data)
{
  ...
  gboolean      enabled, contained, need_update, add;
  guint         length;
  ...
  need_update = (add = (enabled && ! contained)) || (! enabled && contained);

  /* update the list if needed */
  if (need_update)
    {
      /* use a GPtrArray to make it easier to do clean work when reordering the list */
      array = g_ptr_array_sized_new (length + 2);
      length = 0;
      do
        g_ptr_array_add (array, plugins[length]);
      while (plugins[length++] != NULL);

      if (add)
        g_ptr_array_insert (array, 0, (gpointer) action_name);
      else
        {
          length = 0;
          while (g_strcmp0 (plugins[length++], action_name) != 0);
          g_ptr_array_remove_index (array, length - 1);
        }

      /* save the setting */
      value = g_ptr_array_free (array, FALSE);
      MOUSEPAD_SETTING_SET_STRV (ENABLED_PLUGINS, (const gchar *const *) value);
      g_free (value);
    }
