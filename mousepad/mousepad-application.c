...
struct _MousepadApplication
{
  ...
  /* the menus builder */
  GtkBuilder  *builder;
  gchar      **languages_tooltips, **style_schemes_tooltips;
  gsize        n_languages, n_style_schemes;
};
...
mousepad_application_finalize (GObject *object)
{
  ...
  for (n = 0; n < application->n_languages; n++)
    g_free (application->languages_tooltips[n]);
  for (n = 0; n < application->n_style_schemes; n++)
    g_free (application->style_schemes_tooltips[n]);
...
mousepad_application_create_languages_menu (MousepadApplication *application)
{
  ...
  application->languages_tooltips =
    (gchar**) g_ptr_array_steal (tooltips, &(application->n_languages));
...
mousepad_application_create_style_schemes_menu (MousepadApplication *application)
{
  ...
  application->style_schemes_tooltips =
    (gchar**) g_ptr_array_steal (tooltips, &(application->n_style_schemes));
