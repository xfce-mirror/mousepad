                                    ...
                                    GtkAccelMap     *object)
{
  ...
  /* make sure to not crash because of corrupted data, but the action name does not have
   * to be valid (although it will be ignored at the application level if it isn't) */
  action_name = g_strrstr (accel_path, "/");
  if (action_name != NULL && g_strcmp0 (action_name, "/") != 0)
    {
      accel = gtk_accelerator_name (accel_key, accel_mods);
      if (*accel != '\0')
        accels[0] = accel;

      gtk_application_set_accels_for_action (application, action_name + 1, accels);
      g_free (accel);
    }
}
...
mousepad_application_complete_accel_map (GtkApplication *application)
{
  ...
  const gchar  *excluded_actions[] = { "win.edit.delete", "win.edit.select-all" };
