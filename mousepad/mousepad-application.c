...
static void        mousepad_application_toggle_activate           (GSimpleAction            *action,
                                                                   GVariant                 *parameter,
                                                                   gpointer                  data);
static void        mousepad_application_toggle_update             (MousepadApplication      *application,
                                                                   gchar                    *key,
                                                                   GSettings                *settings);
...
static const GActionEntry toggle_actions_space_type[] =
{
...
#define N_SPACE_TYPE G_N_ELEMENTS (toggle_actions_space_type)

static const GActionEntry toggle_actions_space_location[] =
{
...
#define N_SPACE_LOCATION G_N_ELEMENTS (toggle_actions_space_location)

static const GActionEntry* toggle_actions[] =
{
  toggle_actions_space_type,
  toggle_actions_space_location
};
static guint n_toggle_actions[] =
{
  N_SPACE_TYPE,
  N_SPACE_LOCATION
};
#define N_TOGGLE G_N_ELEMENTS (toggle_actions)
...
mousepad_application_startup (GApplication *gapplication)
{
  ...
  for (m = 0; m < N_TOGGLE; m++)
    {
      g_action_map_add_action_entries (G_ACTION_MAP (application), toggle_actions[m],
                                       n_toggle_actions[m], application);

      for (n = 0; n < n_toggle_actions[m]; n++)
        {
          /* tranform the action name in the setting path */
          path = g_strconcat (".", toggle_actions[m][n].name, NULL);
          path = g_strdelimit (path, ".", '/');

          /* sync the action state to its setting */
          mousepad_setting_connect_object (path, G_CALLBACK (mousepad_application_toggle_update),
                                           application, G_CONNECT_SWAPPED);

          /* associate a flag to the action */
          if (toggle_actions[m][n].change_state == mousepad_application_space_type
              || toggle_actions[m][n].change_state == mousepad_application_space_location)
            {
              action = g_action_map_lookup_action (G_ACTION_MAP (application),
                                                   toggle_actions[m][n].name);
              mousepad_object_set_data (G_OBJECT (action), "flag", GUINT_TO_POINTER (1 << n));
            }

          /* initialize the action state */
          if (mousepad_setting_has_path (path))
            {
              state = mousepad_setting_get_boolean (path);
                                      ...
                                      gpointer       data)
{
  ...
  /* retrieve the setting path from the action name */
  path = g_strconcat (".", g_action_get_name (G_ACTION (action)), NULL);
  path = g_strdelimit (path, ".", '/');

  /* save the setting */
  state = g_variant_get_boolean (g_action_get_state (G_ACTION (action)));
  mousepad_setting_set_boolean (path, ! state);
...
static void
mousepad_application_toggle_update (MousepadApplication *application,
                                    gchar               *key,
                                    GSettings           *settings)
{
  gchar    *schema_id, *action_name, *path;
  gboolean  state;

  /* retrieve the action name from the setting schema id */
  g_object_get (settings, "schema_id", &schema_id, NULL);
  action_name = g_strconcat (schema_id + MOUSEPAD_PATH_LEN, ".", key, NULL);
  path = g_strconcat (".", action_name, NULL);
  path = g_strdelimit (path, ".", '/');

  /* get the setting and request for the action state to be changed */
  state = mousepad_setting_get_boolean (path);
  g_action_group_change_action_state (G_ACTION_GROUP (application),
                                      action_name, g_variant_new_boolean (state));
                                 ...
                                 gpointer       data)
{
  ...
  action_all = g_action_map_lookup_action (G_ACTION_MAP (data),
                                           toggle_actions_space_type[N_SPACE_TYPE - 1].name);
      ...
      for (n = 0; n < N_SPACE_TYPE - 1; n++)
        {
          subaction = g_action_map_lookup_action (G_ACTION_MAP (data),
                                                  toggle_actions_space_type[n].name);
                                     ...
                                     gpointer       data)
{
  ...
  action_all = g_action_map_lookup_action (G_ACTION_MAP (data),
                                           toggle_actions_space_location[N_SPACE_LOCATION - 1].name);
  ...
  if (g_variant_get_boolean (state))
    {
      ...
      for (n = 0; n < N_SPACE_LOCATION - 1; n++)
        {
          subaction = g_action_map_lookup_action (G_ACTION_MAP (data),
                                                  toggle_actions_space_location[n].name);
          if (! g_variant_get_boolean (g_action_get_state (subaction)))
            g_action_activate (subaction, NULL);
        }
