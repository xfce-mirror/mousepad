                           ...
                           gboolean          must_exist)
{
  ...
  result = mousepad_file_open (document->file, must_exist, FALSE, make_valid, line, column, &error);
  ...
  if (info == TARGET_TEXT_URI_LIST
      ...
      && gtk_selection_data_get_length (selection_data) > 0)
    {
      ...
      mousepad_window_open_files (window, (GFile **) data, n_pages,
                                  mousepad_encoding_get_default (), 1, 0, TRUE);
  ...
  if (G_LIKELY (filename != NULL))
    {
      ...
      result = mousepad_file_open (document->file, TRUE, FALSE, FALSE, 1, 0, &error);
  ...
  if (G_LIKELY (mousepad_dialogs_open (GTK_WINDOW (window),
                ...
                == GTK_RESPONSE_ACCEPT))
    {
      ...
      for (file = files; file != NULL; file = file->next)
        mousepad_window_open_file (window, file->data, encoding, 1, 0, TRUE);
      ...
      if (G_LIKELY (uri != NULL))
        {
          ...
          file = g_file_new_for_uri (uri);
          succeed = mousepad_window_open_file (window, file, encoding, 1, 0, TRUE);
          g_object_unref (file);
                               ...
                               gpointer       data)
{
  ...
  /* reload the file */
  retval = mousepad_file_open (document->file, TRUE, FALSE, FALSE, 1, 0, &error);
