...
static void              mousepad_window_update_bar_visibility        (MousepadWindow         *window,
                                                                       const gchar            *key);
static gboolean          mousepad_window_menubar_key_event            (MousepadWindow         *window,
                                                                       GdkEventKey            *event,
                                                                       GList                  *mnemonics);
...
# pragma GCC diagnostic pop
  ...
  /* to allow the window to receive the associated button release event */
                                ...
                                gpointer       data)
{
  ...
  if (mousepad_window_get_in_fullscreen (window))
    MOUSEPAD_SETTING_SET_ENUM (MENUBAR_VISIBLE_FULLSCREEN, (active ? YES : NO));
  else
                                ...
                                gpointer       data)
{
  ...
  if (mousepad_window_get_in_fullscreen (window))
    MOUSEPAD_SETTING_SET_ENUM (TOOLBAR_VISIBLE_FULLSCREEN, (active ? YES : NO));
  else
                                  ...
                                  gpointer       data)
{
  ...
  if (mousepad_window_get_in_fullscreen (window))
    MOUSEPAD_SETTING_SET_ENUM (STATUSBAR_VISIBLE_FULLSCREEN, (active ? YES : NO));
  else
                                       ...
                                       const gchar    *hint)
{
  ...
  /* sync the action state to the setting */
  ...
  if (g_strcmp0 (path, MENUBAR) == 0)
    {
      ...
      else if (mnemonics != NULL)
        {
          g_signal_handlers_disconnect_by_data (window, mnemonics);
          g_signal_handlers_disconnect_by_data (window->menubar, mnemonics);
        }
    }
...
static gboolean
mousepad_window_hide_menubar_event (GtkWidget *widget,
                                    GdkEvent  *event,
                                    GList     *mnemonics)
{
  /* disconnect signals and hide the menubar */
  mousepad_disconnect_by_func (widget, mousepad_window_hide_menubar_event, mnemonics);
  if (MOUSEPAD_IS_WINDOW (widget))
    gtk_widget_hide (MOUSEPAD_WINDOW (widget)->menubar);
  else
  ...
  if (event->state & GDK_MOD1_MASK || event->keyval == GDK_KEY_Alt_L
      || (event->keyval == GDK_KEY_Escape && event->type == GDK_KEY_PRESS))
    {
      ...
      if (event->type == GDK_KEY_PRESS
        ...
        {
          /* disconnect signals and hide the menubar */
          mousepad_disconnect_by_func (window, mousepad_window_hide_menubar_event, mnemonics);
          mousepad_disconnect_by_func (window->menubar, mousepad_window_hide_menubar_event, mnemonics);
      ...
      /* show the menubar if Alt key was released or if one of its mnemonic keys matched */
                       ...
                       && g_list_find (mnemonics, GUINT_TO_POINTER (event->keyval)))))
        {
          ...
          g_signal_connect (G_OBJECT (window), "button-press-event",
                            G_CALLBACK (mousepad_window_hide_menubar_event), mnemonics);
          ...
          g_signal_connect (G_OBJECT (window), "scroll-event",
                            G_CALLBACK (mousepad_window_hide_menubar_event), mnemonics);
          g_signal_connect (G_OBJECT (window->menubar), "selection-done",
                            G_CALLBACK (mousepad_window_hide_menubar_event), mnemonics);
