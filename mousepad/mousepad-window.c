                                    ...
                                    GList     *mnemonics)
{
  ...
  if (MOUSEPAD_IS_WINDOW (widget))
    window = MOUSEPAD_WINDOW (widget);
  else
    window = MOUSEPAD_WINDOW (gtk_widget_get_ancestor (widget, MOUSEPAD_TYPE_WINDOW));
      ...
      /* show the menubar if Alt key was released or if one of its mnemonic keys matched */
                       ...
                       && g_list_find (mnemonics, GUINT_TO_POINTER (event->keyval)))))
        {
          ...
          g_signal_connect (G_OBJECT (window), "scroll-event",
                            G_CALLBACK (mousepad_window_hide_menubar_event), mnemonics);
          g_signal_connect (G_OBJECT (window->menubar), "deactivate",
                            G_CALLBACK (mousepad_window_hide_menubar_event), mnemonics);
