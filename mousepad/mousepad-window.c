          ...
          /* radio action */
          else if (param_type != NULL && g_variant_type_equal (state_type, param_type))
            {
              ...
              if (last_button != NULL && g_strcmp0 (action_name, last_action_name) == 0
                  && ! g_variant_equal (target,
                         gtk_actionable_get_action_target_value (GTK_ACTIONABLE (last_button))))
                button = gtk_radio_button_new (gtk_radio_button_get_group (last_button));
              /* a new radio group */
              else
                {
