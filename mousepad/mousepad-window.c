...
mousepad_window_init (MousepadWindow *window)
{
  ...
  /* catch some events */
  window->controller_scroll = gtk_event_controller_scroll_new (GTK_EVENT_CONTROLLER_SCROLL_DISCRETE
      ...
      for (i = 0; i < n_pages; i++)
        {
          ...
          GtkWidget *box = gtk_widget_get_parent (gtk_notebook_get_tab_label (GTK_NOTEBOOK (window->notebook), page));
          GtkAllocation alloc;
