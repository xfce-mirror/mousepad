  ...
  if (G_LIKELY (provider->module != NULL))
    {
      ...
      if (g_module_symbol (provider->module, "mousepad_plugin_initialize",
          ...
          && g_module_symbol (provider->module, "mousepad_plugin_get_accel",
                              (gpointer *) &(provider->get_accel))
          && g_module_symbol (provider->module, "mousepad_plugin_get_types",
                              (gpointer *) &(provider->get_types)))
        {
          provider->initialize (provider);
          provider->destroyable = provider->is_destroyable ();
          provider->label = provider->get_label ();
          provider->category = provider->get_category ();
          provider->accel = provider->get_accel ();
