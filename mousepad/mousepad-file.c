  ...
  if (G_LIKELY (contents))
    {
      ...
      etag = g_strdup (file->etag);
      g_free (file->etag);
      if (! g_file_replace_contents (file->location, contents, length, etag, TRUE,
                                     G_FILE_CREATE_NONE, &(file->etag), NULL, error))
        goto failed;
                                       ...
                                       GError       **error)
{
  ...
  /* set an error */
  if (error != NULL)
    g_set_error (error, G_FILE_ERROR, error_code, _("Failed to read the status of \"%s\""),
                 mousepad_file_get_path (file));
