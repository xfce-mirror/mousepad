...
mousepad_encoding_dialog_test_encodings_idle (gpointer user_data)
{
  ...
  GError                 *error = NULL;
  const gchar            *system_charset, *subtitle;
  ...
  if (! g_file_load_contents (mousepad_file_get_location (dialog->document->file),
    ...
    {
      /* show the warning */
      mousepad_dialogs_show_error (GTK_WINDOW (dialog), error, _("Failed to open the document"));
  ...
  for (i = 0, n = 0; i < n_encoding_infos && ! dialog->cancel_testing; i++)
    {
      ...
      if (encoding_infos[i].encoding == MOUSEPAD_ENCODING_UTF_8
          || g_strcmp0 (encoding_infos[i].charset, system_charset) == 0)
        continue;
