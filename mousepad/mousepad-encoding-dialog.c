...
mousepad_encoding_dialog_test_encodings_idle (gpointer user_data)
{
  ...
  GError                 *error = NULL;
  const gchar            *default_charset, *system_charset, *subtitle;
  ...
  if (! g_file_load_contents (mousepad_file_get_location (dialog->document->file),
    ...
    {
      /* show the warning */
      mousepad_dialogs_show_error (GTK_WINDOW (dialog), error, IO_ERROR_MESSAGE);
  ...
  default_charset = mousepad_encoding_get_charset (default_encoding);
  label = default_valid ? g_strdup_printf ("%s (%s)", _("Default"), default_charset)
                        : g_strdup_printf (_("%s (%s, partial)"), _("Default"), default_charset);
  ...
  if (dialog->radio_system != NULL)
    {
      ...
      encoded = g_convert (contents, length, "UTF-8", system_charset, NULL, &written, NULL);

      /* set the button label accordingly */
      if (encoded == NULL)
        label = g_strdup_printf (_("%s (%s, failed)"), _("System"), system_charset);
      else if (! g_utf8_validate (encoded, written, NULL))
        label = g_strdup_printf (_("%s (%s, partial)"), _("System"), system_charset);
      else
        {
          system_valid = TRUE;
  ...
  for (i = 0, n = 0; i < n_encoding_infos && ! dialog->cancel_testing; i++)
    {
      ...
      if (encoding_infos[i].encoding == default_encoding
          || encoding_infos[i].encoding == system_encoding)
        continue;
