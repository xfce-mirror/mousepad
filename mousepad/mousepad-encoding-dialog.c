  ...
  if (G_LIKELY (encoding))
    {
      ...
      result = mousepad_file_open (dialog->document->file, DEFAULT_LINE, DEFAULT_COLUMN, TRUE, TRUE, TRUE, &error);
    }
  ...
  if (mousepad_file_get_encoding (file) == MOUSEPAD_ENCODING_NONE)
    {
      ...
      result = mousepad_file_open (file, DEFAULT_LINE, DEFAULT_COLUMN, TRUE, TRUE, FALSE, &error);
